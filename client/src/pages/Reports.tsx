...
export default function Reports() {
  ...
  return <div dir="rtl" className="mx-auto max-w-7xl space-y-6 print:bg-white">
    ...
    {query.isLoading && !data ? <div className="soft-card p-12 text-center text-sm text-muted-foreground">جارٍ إعداد التقرير…</div> : data ? <>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{cards.map(card => <article key={card.label} className={`rounded-2xl p-4 shadow-sm ${card.tone}`}><div className="flex items-center justify-between"><p className="text-xs font-bold opacity-80">{card.label}</p><card.icon className="h-5 w-5 opacity-80" /></div><p className="mt-3 text-2xl font-black">{card.value}</p></article>)}</div>
