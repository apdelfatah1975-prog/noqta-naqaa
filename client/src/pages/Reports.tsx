...
export default function Reports() {
  ...
  const exportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, XLSX.utils.json_to_sheet([
      ...
      { البيان: "البنزين — مصروف تشغيل", "الإجمالي": financial.gasolineExpenses / 100 },
      { البيان: "مشتريات المخزن", "الإجمالي": financial.inventoryPurchaseExpenses / 100 },
      { البيان: "المصروفات العامة", "الإجمالي": financial.generalExpenses / 100 },
      { البيان: "مصروفات أخرى أو غير مصنفة", "الإجمالي": financial.uncategorizedExpenses / 100 },
      { البيان: "إجمالي المصروفات غير رواتب الفنيين", "الإجمالي": financial.otherExpenses / 100 },
      { البيان: "صافي إيراد الشركة بعد المصروفات", "الإجمالي": financial.companyNet / 100 },
    ]), "مالية الشركة");
  ...
  const cards = useMemo(() => data ? [
    ...
    { label: "الإيرادات", description: "إجمالي الأموال الداخلة من الزيارات", value: money(data.summary.income), unit: "ريال", tone: "bg-emerald-50 text-emerald-900", icon: WalletCards },
    { label: "المصروفات", description: "إجمالي الأموال الخارجة خلال الفترة", value: money(data.summary.expense), unit: "ريال", tone: "bg-amber-50 text-amber-950", icon: WalletCards },
    { label: "صافي الفترة", description: "إيرادات الفترة ناقص مصروفات الفترة", value: money(data.summary.balance), unit: "ريال", tone: "bg-slate-950 text-white", icon: FileBarChart },
    { label: "رصيد الخزنة الفعلي", description: "الرصيد من جميع حركات الخزنة حتى الآن", value: money(data.summary.treasuryBalance ?? 0), unit: "ريال", tone: "bg-cyan-50 text-cyan-950", icon: WalletCards },
  ...
  return <div className="mx-auto max-w-7xl space-y-6 print:bg-white">
    ...
    {query.isLoading && !data ? <div className="soft-card p-12 text-center text-sm text-muted-foreground">جارٍ إعداد التقرير…</div> : data ? <>
      ...
      <section className="soft-card border border-cyan-200 bg-cyan-50/70 p-4 print:hidden"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-base font-black text-cyan-950">مطابقة التقرير مع الخزنة</h2><p className="mt-1 text-xs leading-5 text-cyan-900/80">صافي الفترة = الإيرادات − المصروفات. رصيد الخزنة الفعلي يتأثر بكل الحركات حتى الآن، لذلك قد يختلف عند اختيار فترة جزئية.</p></div><span className={`w-fit rounded-full px-3 py-1 text-xs font-black ${reconciliationMatches ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-900"}`}>{reconciliationMatches ? "المعادلة متطابقة" : "راجع الفترة المحددة"}</span></div><div className="mt-3 grid gap-2 sm:grid-cols-4"><div className="rounded-lg bg-white/80 p-3"><p className="text-[11px] font-bold text-muted-foreground">الإيرادات</p><p className="mt-1 text-base font-black text-emerald-800">{money(data.summary.income)} ريال</p></div><div className="rounded-lg bg-white/80 p-3"><p className="text-[11px] font-bold text-muted-foreground">المصروفات</p><p className="mt-1 text-base font-black text-amber-800">{money(data.summary.expense)} ريال</p></div><div className="rounded-lg bg-white/80 p-3"><p className="text-[11px] font-bold text-muted-foreground">صافي الفترة</p><p className="mt-1 text-base font-black text-slate-900">{money(periodReconciliation)} ريال</p></div><div className="rounded-lg bg-white/80 p-3"><p className="text-[11px] font-bold text-muted-foreground">رصيد الخزنة الحالي</p><p className="mt-1 text-base font-black text-cyan-900">{money(treasuryBalance)} ريال</p></div></div></section>
...
function Metric({ label, helper, value }: { label: string; helper?: string; value: string }) { return <div className="rounded-lg border border-teal-950/8 bg-white px-3 py-2.5"><p className="text-xs font-bold text-muted-foreground">{label}</p><p className="mt-1 text-base font-black text-teal-950">{value} <span className="text-[10px] font-bold text-muted-foreground">ريال</span></p><p className="mt-1 text-[10px] leading-4 text-muted-foreground">{helper ?? "إجمالي خلال الفترة المحددة"}</p></div>; }
