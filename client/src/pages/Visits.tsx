...
export default function Visits() {
  ...
  const [collectedCurrency, setCollectedCurrency] = useState<Currency>("EGP");
  const utils = trpc.useUtils();
  const createVisit = trpc.filters.visits.create.useMutation({ onSuccess: result => { utils.filters.dashboard.invalidate(); utils.filters.customers.list.invalidate(); utils.filters.customers.get.invalidate(); utils.filters.reminders.due.invalidate(); toast.success(result.reminderCreated ? "تم التسجيل وإنشاء تذكير تلقائي بعد 120 يومًا" : "تم تسجيل الزيارة بنجاح"); setNotes(""); setCollectedAmount(""); setCollectedCurrency("EGP"); }, onError: error => toast.error(error.message || "تعذر تسجيل الزيارة. يرجى المحاولة مرة أخرى.") });
