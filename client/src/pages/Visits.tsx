...
export default function Visits() {
  ...
  return <div className="-mx-4 -mt-2 w-[calc(100%+2rem)] max-w-none space-y-4 px-0 sm:-mx-6 sm:w-[calc(100%+3rem)] lg:-mx-8 lg:w-[calc(100%+4rem)]">
    <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><div className="flex items-center gap-2"><span className="grid h-9 w-9 place-items-center rounded-xl bg-teal-100 text-teal-800"><ClipboardList className="h-5 w-5" /></span><div><h1 className="page-heading">سجل الزيارات</h1><p className="page-subheading">كل عميل تمت زيارته وما تم إنجازه له، في جدول واحد.</p></div></div></div><div className="flex items-center gap-2 self-start sm:self-end"><span className="rounded-xl bg-teal-100 px-3 py-2 text-sm font-black text-teal-800">{rows.length.toLocaleString("ar-SA")} زيارة</span><span className={`rounded-xl px-3 py-2 text-sm font-bold ${online ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>{online ? "متصل" : "دون نت"}</span></div></div>
...
function VisitHistory({ rows, search, onSearchChange, typeFilter, onTypeFilterChange, dateFrom, onDateFromChange, dateTo, onDateToChange, onClearFilters, onOpenCustomer, onDelete }: { rows: VisitRow[]; search: string; onSearchChange: (value: string) => void; typeFilter: string; onTypeFilterChange: (value: string) => void; dateFrom: string; onDateFromChange: (value: string) => void; dateTo: string; onDateToChange: (value: string) => void; onClearFilters: () => void; onOpenCustomer: (customer: any) => void; onDelete: (visit: VisitRow) => void }) {
  ...
  return <section className="soft-card overflow-hidden">
    <div className="border-b border-teal-950/6 bg-white p-4 sm:p-5">
      <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-black text-teal-950">الزيارات المنفذة</h2><p className="mt-1 text-sm text-muted-foreground">يعرض هذا السجل كل عميل تمت زيارته ونوع الخدمة وما تم إنجازه له.</p></div><span className="rounded-full bg-teal-100 px-3 py-1.5 text-xs font-extrabold text-teal-900" aria-live="polite">النتائج: {rows.length.toLocaleString("ar-SA")}</span></div>
