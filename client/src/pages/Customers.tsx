...
export default function Customers() {
  ...
  function submit(event: FormEvent) {
    ...
    const payload = { ...form, address: form.address || null, latitude: form.latitude || null, longitude: form.longitude || null, notes: form.notes || null };
    if (form.id) {
      if (isOffline) return toast.error("تعديل البيانات يحتاج اتصالًا بالإنترنت حاليًا.");
      updateCustomer.mutate(payload as CustomerForm & { id: number });
      return;
    }
    if (isOffline) {
      ...
      queueOfflineCustomer(offlineUser.id, payload);
