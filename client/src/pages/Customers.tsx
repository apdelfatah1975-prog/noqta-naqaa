...
type CustomerForm = { id?: number; manualCode: string; name: string; phone: string; address: string; latitude: string; longitude: string; notes: string; firstVisitType: VisitType; firstVisitDate: string; firstTechnicianName: string; firstVisitNotes: string; firstCollectedAmount: string };
...
const emptyCustomer: CustomerForm = { manualCode: "", name: "", phone: "", address: "", latitude: "", longitude: "", notes: "", firstVisitType: "installation", firstVisitDate: toDateTimeLocal(), firstTechnicianName: "", firstVisitNotes: "", firstCollectedAmount: "" };
...
export default function Customers() {
  ...
  const [collectedAmountMin, setCollectedAmountMin] = useState("");
  const [collectedAmountMax, setCollectedAmountMax] = useState("");
  ...
  const input = useMemo(() => ({
    ...
    collectedAmountMin: collectedAmountMin.trim() ? Math.round(Number(collectedAmountMin) * 100) : undefined,
    collectedAmountMax: collectedAmountMax.trim() ? Math.round(Number(collectedAmountMax) * 100) : undefined,
    sortBy,
  }), [search, followUpStatus,     followUpDate, visitDateFrom, visitDateTo, collectedAmountMin, collectedAmountMax, sortBy]);
  ...
  function openEdit(customer: NonNullable<typeof customers>[number]) { const serviceDate = customer.followUp?.lastServiceVisitDate ? new Date(customer.followUp.lastServiceVisitDate) : new Date(); serviceDate.setMinutes(serviceDate.getMinutes() - serviceDate.getTimezoneOffset()); setForm({ ...emptyCustomer, id: customer.id, manualCode: customer.manualCode || "", name: customer.name, phone: customer.phone, address: customer.address || "", latitude: customer.latitude || "", longitude: customer.longitude || "", notes: customer.notes || "", firstVisitDate: serviceDate.toISOString().slice(0, 16) }); setDialogOpen(true); }
  function submit(event: FormEvent) {
    ...
    if (form.id) {
      ...
      setPendingUpdate({ id: form.id, manualCode: form.manualCode.trim() || null, name: form.name, phone: form.phone, address: form.address || null, latitude: form.latitude || null, longitude: form.longitude || null, notes: form.notes || null, serviceDate: new Date(form.firstVisitDate), ...(form.firstCollectedAmount.trim() ? { collectedAmount: Math.round(Number(form.firstCollectedAmount) * 100) } : {}) });
      setPinOpen(true);
      return;
    }
    if (isOffline) {
