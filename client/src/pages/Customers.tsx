...
type CustomerForm = { id?: number; name: string; phone: string; address: string; latitude: string; longitude: string; notes: string; firstVisitType: VisitType; firstVisitDate: string; firstTechnicianName: string; firstVisitNotes: string; firstCollectedAmount: string; firstCollectedCurrency: Currency };
...
const emptyCustomer: CustomerForm = { name: "", phone: "", address: "", latitude: "", longitude: "", notes: "", firstVisitType: "installation", firstVisitDate: toDateTimeLocal(), firstTechnicianName: "", firstVisitNotes: "", firstCollectedAmount: "", firstCollectedCurrency: "SAR" };
...
export default function Customers() {
  ...
  function openEdit(customer: NonNullable<typeof customers>[number]) { setForm({ ...emptyCustomer, id: customer.id, name: customer.name, phone: customer.phone, address: customer.address || "", latitude: customer.latitude || "", longitude: customer.longitude || "", notes: customer.notes || "" }); setDialogOpen(true); }
  function submit(event: FormEvent) {
    event.preventDefault();
    const payload = { name: form.name, phone: form.phone, address: form.address || null, latitude: form.latitude || null, longitude: form.longitude || null, notes: form.notes || null, ...(form.id ? {} : { firstVisitType: form.firstVisitType, firstVisitDate: new Date(form.firstVisitDate), firstTechnicianName: form.firstTechnicianName || null, firstVisitNotes: form.firstVisitNotes || null, firstCollectedAmount: Math.round(Number(form.firstCollectedAmount || 0) * 100), firstCollectedCurrency: form.firstCollectedCurrency }) };
    if (form.id) {
      if (isOffline) return toast.error("تعديل البيانات يحتاج اتصالًا بالإنترنت حاليًا.");
      updateCustomer.mutate({ id: form.id, name: form.name, phone: form.phone, address: form.address || null, latitude: form.latitude || null, longitude: form.longitude || null, notes: form.notes || null });
      return;
    }
    if (isOffline) {
