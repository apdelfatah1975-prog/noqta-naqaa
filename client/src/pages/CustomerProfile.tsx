...
export default function CustomerProfile() {
  ...
  useEffect(() => {
    ...
    if (shouldOpenVisit && data) {
      ...
      setNotes(""); setVisitResult(""); setVisitTechnicianName(""); setVisitCollectedAmount(""); setVisitTdsIn(""); setVisitTdsOut(""); setVisitItems([]); setManualItemName(""); setManualItemQuantity("1");
  ...
  function submitVisit(event: FormEvent) {
    ...
    const payload = { customerId, visitType, visitDate: new Date(visitDate), technicianName: visitTechnicianName || null, tdsIn: visitTdsIn.trim() ? Math.round(Number(visitTdsIn)) : null, tdsOut: visitTdsOut.trim() ? Math.round(Number(visitTdsOut)) : null, visitResult: visitResult || null, collectedAmount, collectedCurrency: "SAR" as const, notes: notes || null, items: visitItems.filter(item => item.quantity > 0) };
    ...
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      <section className="soft-card overflow-hidden">
        <div className="bg-[linear-gradient(135deg,#064e4a,#0f766e)] p-6 text-white sm:flex sm:items-center sm:justify-between">
          ...
          <Button onClick={() => {       setVisitType("maintenance"); setVisitDate(toDateTimeLocal()); setNotes(""); setVisitResult(""); setVisitTechnicianName(""); setVisitCollectedAmount(""); setVisitTdsIn(""); setVisitTdsOut(""); setVisitItems([]); setManualItemName(""); setManualItemQuantity("1"); setDialogOpen(true); }} className="mt-5 rounded-xl bg-white text-teal-800 hover:bg-teal-50 sm:mt-0"><CalendarPlus className="ml-2 h-4 w-4" />تسجيل زيارة</Button>
        </div>
