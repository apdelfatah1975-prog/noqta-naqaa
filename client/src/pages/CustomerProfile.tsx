...
import { ArrowRight, BellRing, CalendarClock, CalendarPlus, Loader2, MapPinned, Phone } from "lucide-react";
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      <section className="grid gap-6 lg:grid-cols-3">
        <div className="soft-card lg:col-span-2"><div className="flex items-center justify-between border-b border-teal-950/6 p-5"><div><h2 className="font-extrabold">سجل الزيارات</h2><p className="mt-1 text-xs text-muted-foreground">جميع الزيارات المسجلة للعميل مرتبة من الأحدث.</p></div><span className="rounded-full bg-teal-50 px-3 py-1 text-xs font-bold text-teal-800">{data.visits.length} زيارة</span></div><div className="divide-y divide-teal-950/6">{data.visits.length ? data.visits.map(visit => <div key={visit.id} className="flex gap-4 p-5"><div className="mt-1 h-3 w-3 shrink-0 rounded-full bg-teal-500 ring-4 ring-teal-50" /><div className="min-w-0 flex-1"><div className="flex flex-wrap items-center justify-between gap-2"><Badge variant="secondary" className="bg-teal-50 text-teal-800">{visitTypeLabels[visit.visitType]}</Badge><p className="text-xs font-bold text-muted-foreground">{formatDateTime(visit.visitDate)}</p></div>{visit.notes ? <p className="mt-2 text-sm leading-6 text-muted-foreground">{visit.notes}</p> : null}</div></div>) : <div className="p-10 text-center text-sm text-muted-foreground">لا توجد زيارات سابقة لهذا العميل.</div>}</div></div>
