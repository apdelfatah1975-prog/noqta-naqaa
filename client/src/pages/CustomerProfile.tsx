...
export default function CustomerProfile() {
  ...
  useEffect(() => {
    ...
    if (shouldOpenVisit && data) {
      ...
      setNotes(""); setVisitResult(""); setVisitTechnicianName(""); setVisitCollectedAmount(""); setVisitTdsIn(""); setVisitTdsOut(""); setVisitNextDate(""); setVisitItems([]); setManualItemName(""); setManualItemQuantity("1");
  ...
  function submitVisit(event: FormEvent) {
    ...
    const payload = { customerId, visitType, visitDate: new Date(visitDate), nextVisitDate: visitNextDate ? new Date(visitNextDate) : null, technicianName: visitTechnicianName || null, tdsIn: visitTdsIn.trim() ? Math.round(Number(visitTdsIn)) : null, tdsOut: visitTdsOut.trim() ? Math.round(Number(visitTdsOut)) : null, visitResult: visitResult || null, collectedAmount, collectedCurrency: "SAR" as const, notes: notes || null, items: visitItems.filter(item => item.quantity > 0) };
