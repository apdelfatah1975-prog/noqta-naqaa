...
export const formatPayrollMoney = (amount: number) => new Intl.NumberFormat("ar-SA", { maximumFractionDigits: 0, minimumFractionDigits: 0 }).format(Math.round(amount / 100));
...
export default function TechnicianPayroll() {
  ...
  return <div dir="rtl" className="mx-auto max-w-7xl space-y-4 print:bg-white">
    ...
    <section className="soft-card overflow-hidden border-cyan-200/70 print:hidden"><div className="border-b border-cyan-950/6 bg-gradient-to-l from-cyan-50 via-white to-teal-50 p-3 sm:p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-bold text-cyan-700">لوحة متابعة شهرية</p><h2 className="mt-1 text-xl font-black text-cyan-950">إجمالي العمولات المستحقة</h2><p className="mt-1 text-xs leading-6 text-muted-foreground">الفترة المحددة: <span dir="ltr">{bounds.from} — {bounds.to}</span></p></div><div className="rounded-2xl bg-cyan-700 px-4 py-3 text-center text-white"><p className="text-[11px] font-bold text-cyan-100">إجمالي عمولات الشهر</p><p className="mt-1 text-2xl font-black">{formatPayrollMoney(technicianCommissionTotal + salesCommissionTotal)} ريال</p></div></div></div><div className="grid gap-3 p-3 sm:grid-cols-2 sm:p-4"><article className="rounded-2xl border border-teal-100 bg-teal-50/70 p-3"><div className="flex items-center justify-between gap-2"><div><p className="text-xs font-bold text-teal-800">عمولات الفنيين</p><p className="mt-1 text-2xl font-black text-teal-950">{formatPayrollMoney(technicianCommissionTotal)} ريال</p></div><span className="rounded-full bg-white px-2.5 py-1 text-xs font-black text-teal-800">{selected.length.toLocaleString("ar-SA")} فني</span></div><div className="mt-3 space-y-1.5">{selected.filter(row => row.transactions.some(item => item.category.includes("عمولة"))).map(row => <div key={`commission-${row.technician}`} className="flex items-center justify-between gap-2 border-b border-teal-950/6 pb-1.5 text-sm"><span className="font-bold text-teal-900">{row.technician}</span><span className="font-black text-teal-700">{formatPayrollMoney(row.transactions.filter(item => item.category.includes("عمولة")).reduce((sum, item) => sum + item.amount, 0))} ريال</span></div>)}{technicianCommissionTotal === 0 ? <p className="text-xs font-semibold text-muted-foreground">لا توجد عمولات فنيين مسجلة في هذا الشهر.</p> : null}</div></article><article className="rounded-2xl border border-cyan-100 bg-cyan-50/70 p-3"><div className="flex items-center justify-between gap-2"><div><p className="text-xs font-bold text-cyan-800">عمولات متابعي العملاء</p><p className="mt-1 text-2xl font-black text-cyan-950">{formatPayrollMoney(salesCommissionTotal)} ريال</p></div><span className="rounded-full bg-white px-2.5 py-1 text-xs font-black text-cyan-800">{salesAgentRows.length.toLocaleString("ar-SA")} متابع</span></div><div className="mt-3 space-y-1.5">{salesAgentRows.filter(row => row.commission > 0).map(row => <div key={`sales-commission-${row.name}`} className="flex items-center justify-between gap-2 border-b border-cyan-950/6 pb-1.5 text-sm"><span className="font-bold text-cyan-900">{row.name}</span><span className="font-black text-cyan-700">{formatPayrollMoney(row.commission)} ريال</span></div>)}{salesCommissionTotal === 0 ? <p className="text-xs font-semibold text-muted-foreground">لا توجد عمولات متابعة مستحقة في هذا الشهر.</p> : null}</div></article></div></section>
