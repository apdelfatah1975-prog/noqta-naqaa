...
export default function TechnicianPayroll() {
  ...
  return <div dir="rtl" className="mx-auto max-w-7xl space-y-6 print:bg-white">
    ...
    <section className="soft-card overflow-hidden p-5 sm:p-6"><div className="mb-5 flex items-center justify-between gap-3"><div><h2 className="text-lg font-black text-teal-950">ملخص الفنيين</h2><p className="mt-1 text-xs leading-6 text-muted-foreground">المستحق يشمل الراتب والعمولات، والمدفوع يأتي من حركات الخزينة فقط.</p></div><WalletCards className="h-6 w-6 shrink-0 text-teal-700" /></div><div className="grid gap-3 md:hidden">{selected.length ? selected.map(row => <article key={`mobile-${row.technician}`} className="rounded-2xl border border-teal-100 bg-gradient-to-br from-white to-teal-50/60 p-4 shadow-sm"><div className="flex items-start justify-between gap-3"><div><h3 className="text-base font-black text-teal-950">{row.technician}</h3><span className={row.status === "paid" ? "mt-2 inline-flex rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-black text-emerald-800" : "mt-2 inline-flex rounded-full bg-amber-100 px-2.5 py-1 text-xs font-black text-amber-800"}>{row.status === "paid" ? "تمت التسوية" : "متبقي"}</span></div><Button type="button" className="h-9 rounded-xl bg-teal-700 px-3 text-xs font-bold hover:bg-teal-800" onClick={() => { setPaymentTechnician(row.technician); setPaymentOpen(true); }}>تسجيل دفعة</Button></div><div className="mt-4 grid grid-cols-3 gap-2 text-center"><div className="rounded-xl bg-indigo-50 p-2"><p className="text-[11px] font-bold text-indigo-800">المستحق</p><p className="mt-1 text-sm font-black text-indigo-950">{money(row.required)}</p></div><div className="rounded-xl bg-emerald-50 p-2"><p className="text-[11px] font-bold text-emerald-800">المدفوع</p><p className="mt-1 text-sm font-black text-emerald-950">{money(row.paid)}</p></div><div className="rounded-xl bg-amber-50 p-2"><p className="text-[11px] font-bold text-amber-800">المتبقي</p><p className="mt-1 text-sm font-black text-amber-950">{money(row.remaining)}</p></div></div><p className="mt-3 text-xs font-semibold text-muted-foreground">عدد العمليات: {row.transactions.length}</p></article>) : <div className="rounded-2xl bg-slate-50 p-6 text-center text-sm text-muted-foreground">لا توجد عمليات رواتب في هذا الشهر.</div>}</div><div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[760px] text-right text-sm"><thead className="rounded-xl bg-teal-50 text-xs font-bold text-teal-950"><tr><th className="px-3 py-3">الفني</th><th className="px-3 py-3">الحالة</th><th className="px-3 py-3">المستحق</th><th className="px-3 py-3">المدفوع</th><th className="px-3 py-3">المتبقي</th><th className="px-3 py-3">العمليات</th></tr></thead><tbody className="divide-y divide-teal-950/6">{selected.length ? selected.map(row => <tr key={row.technician} className="transition hover:bg-teal-50/40"><td className="px-3 py-4 font-black text-teal-950">{row.technician}</td><td className="px-3 py-4"><span className={row.status === "paid" ? "inline-flex rounded-full bg-emerald-100 px-3 py-1 text-xs font-black text-emerald-800" : "inline-flex rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-800"}>{row.status === "paid" ? "مدفوع" : "متبقي"}</span></td><td className="px-3 py-4 font-bold">{money(row.required)}</td><td className="px-3 py-4 font-black text-teal-800">{money(row.paid)}</td><td className="px-3 py-4 font-black text-amber-700">{money(row.remaining)}</td><td className="px-3 py-4"><div className="flex flex-wrap items-center gap-2"><span className="font-bold text-muted-foreground">{row.transactions.length}</span><Button type="button" className="h-8 rounded-lg bg-teal-700 px-2.5 text-xs font-bold hover:bg-teal-800" onClick={() => { setPaymentTechnician(row.technician); setPaymentOpen(true); }}>تسجيل دفعة</Button></div></td></tr>) : <tr><td colSpan={6} className="p-10 text-center text-muted-foreground">لا توجد عمليات رواتب في هذا الشهر.</td></tr>}</tbody></table></div></section>
