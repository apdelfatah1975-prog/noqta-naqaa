...
import { AlertTriangle, ArrowDownLeft, ArrowUpRight, Boxes, Droplets, Filter, PackagePlus, PackageSearch, Pencil, Plus, Refrigerator, Snowflake } from "lucide-react";
...
export default function Inventory() {
  ...
  const updateItem = trpc.filters.inventory.updateItem.useMutation({
    onSuccess: () => {
      utils.filters.inventory.summary.invalidate();
      utils.filters.dashboard.invalidate();
      utils.filters.cash.summary.invalidate();
      setItemDialog(false); setEditingItemId(null); resetItemForm();
      toast.success("تم تعديل بيانات الصنف بدون إنشاء صنف جديد");
    },
    onError: error => toast.error(error.message || "تعذر تعديل الصنف."),
  });
  ...
  function resetItemForm() {
    setItemName(""); setItemCategory("مستلزمات تركيب"); setItemUnit("قطعة"); setReorderLevel("2"); setDefaultUnitCost("0"); setOpeningQuantity("0"); setItemNotes("");
  }
  function openEditItem(item: typeof data.items[number]) {
    setEditingItemId(item.id);
    setItemName(item.name);
    setItemCategory(item.category || "مستلزمات تركيب");
    setItemUnit(item.unit || "قطعة");
    setReorderLevel(String(item.reorderLevel ?? 2));
    setDefaultUnitCost(String((item.defaultUnitCost ?? 0) / 100));
    setOpeningQuantity(String(item.openingQuantity ?? 0));
  ...
  function submitItem(event: FormEvent) {
    ...
    if (editingItemId) {
      updateItem.mutate({ inventoryItemId: editingItemId, name: input.name, category: input.category, unit: input.unit, reorderLevel: input.reorderLevel, defaultUnitCost: input.defaultUnitCost, notes: input.notes });
      return;
    }
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        ...
        <div className="flex flex-wrap gap-2">
          <Button onClick={() => { setEditingItemId(null); resetItemForm(); setItemDialog(true); }} className="h-11 rounded-xl bg-teal-700 px-5 font-bold hover:bg-teal-800"><Plus className="ml-2 h-5 w-5" />إضافة صنف جديد</Button>
        ...
        <div className="hidden overflow-x-auto md:block">
          <table className="w-full min-w-[760px] text-right">
            ...
            <tbody className="divide-y divide-teal-950/6">
              {data?.items.length ? data.items.map(item => <InventoryTableRow key={item.id} item={item} selected={item.id === selectedItemId} onMovement={() => openMovement({ id: item.id, name: item.name, defaultUnitCost: item.defaultUnitCost }, "outgoing")} onIncoming={() => openMovement({ id: item.id, name: item.name, defaultUnitCost: item.defaultUnitCost }, "incoming")} onEdit={() => openEditItem(item)} onDelete={() => setPinAction({ kind: "item", id: item.id })} />) : <EmptyInventoryRow isLoading={isLoading} />}
            </tbody>
          </table>
        </div>
          ...
          {data?.items.length ? data.items.map(item => (
            <div data-inventory-item-id={item.id} key={item.id} className={`inventory-item-row space-y-4 p-5 transition-[background-color,box-shadow] duration-500 ease-out ${item.id === selectedItemId ? "inventory-item-row-selected bg-orange-100 ring-2 ring-inset ring-orange-500 shadow-[inset_0_0_0_1px_rgba(249,115,22,.45)]" : "hover:bg-teal-50/40"}`}>
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                ...
                <div className="grid grid-cols-2 gap-2"><Button size="sm" variant="outline" onClick={() => openEditItem(item)} className="w-full rounded-xl border-sky-200 text-sky-800 hover:bg-sky-50"><Pencil className="ml-1 h-4 w-4" />تعديل الصنف</Button><Button size="sm" variant="outline" onClick={() => setPinAction({ kind: "item", id: item.id })} className="w-full rounded-xl border-rose-200 text-rose-700 hover:bg-rose-50">حذف الصنف</Button></div>
              </div>
            </div>
      ...
      <section className="soft-card overflow-hidden">
        ...
        <div className="divide-y divide-teal-950/6 md:hidden">
          {data?.movements.length ? data.movements.map(movement => <div key={movement.id} className="p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-xs text-muted-foreground">الصنف</p><p className="mt-1 font-extrabold text-teal-950">{movement.inventoryItemName}</p></div><span className="font-extrabold">{movement.quantity} قطعة</span></div><div className="mt-3"><MovementType movementType={movement.movementType} /></div><div className="mt-3 grid grid-cols-2 gap-2 text-xs"><div className="rounded-xl bg-slate-50 p-2.5"><p className="text-[11px] font-bold text-slate-500">التاريخ</p><p className="mt-1 font-bold text-teal-950">{formatDate(movement.movementDate)}</p></div>{movement.movementType === "incoming" ? <><div className="col-span-2 flex items-center justify-between gap-3 rounded-xl border border-violet-200 bg-violet-50 px-3 py-2"><div><p className="text-[10px] font-bold text-violet-700">سعر الوحدة</p><p className="mt-0.5 text-sm font-black text-violet-950">{formatMoney(movement.unitCost ?? 0)}</p></div><div className="h-7 w-px bg-violet-200" /><div className="text-left"><p className="text-[10px] font-bold text-violet-700">الإجمالي · {movement.quantity} قطعة</p><p className="mt-0.5 text-sm font-black text-violet-950">{formatMoney((movement.unitCost ?? 0) * movement.quantity)}</p></div></div></> : null}<div className="rounded-xl bg-slate-50 p-2.5"><p className="text-[11px] font-bold text-slate-500">الفني / المستلم</p><p className="mt-1 font-bold text-teal-950">{movement.technicianName || "—"}</p></div>{movement.notes ? <div className="col-span-2 rounded-xl bg-slate-50 p-2.5"><p className="text-[11px] font-bold text-slate-500">ملاحظات</p><p className="mt-1 font-bold text-teal-950">{movement.notes}</p></div> : null}</div><Button size="sm" variant="outline" onClick={() => setPinAction({ kind: "movement", id: movement.id })} className="mt-4 w-full rounded-xl border-rose-200 text-rose-700 hover:bg-rose-50">حذف الحركة</Button></div>) : <div className="p-10 text-center text-sm text-muted-foreground">لا توجد حركات في المخزن بعد.</div>}
        </div>
      </section>
      ...
      <Dialog open={itemDialog} onOpenChange={open => { setItemDialog(open); if (!open) { setEditingItemId(null); resetItemForm(); } }}><DialogContent dir="rtl"><DialogHeader><DialogTitle>{editingItemId ? "تعديل بيانات الصنف" : "إضافة صنف جديد إلى المخزن"}</DialogTitle><p className="text-sm text-muted-foreground">{editingItemId ? "عدّل اسم الصنف أو بياناته ثم احفظ. الرصيد والحركات السابقة لن تتغير." : "اكتب اسم الصنف والكمية، ثم احفظ. باقي البيانات اختيارية ويمكن تعديلها لاحقًا."}</p></DialogHeader><form onSubmit={submitItem} className="space-y-4 py-2"><div className="grid gap-4 sm:grid-cols-2"><label><span className="field-label">اسم الصنف</span><input className="field-input" value={itemName} onChange={event => setItemName(event.target.value)} required placeholder="مثال: شمعة كربون أو فلتر جامبو" /></label><label><span className="field-label">وحدة القياس</span><input className="field-input" value={itemUnit} onChange={event => setItemUnit(event.target.value)} placeholder="قطعة" /></label>{editingItemId ? <label><span className="field-label">الرصيد الافتتاحي</span><input type="number" className="field-input bg-slate-50" value={openingQuantity} readOnly /><p className="mt-1 text-xs text-muted-foreground">لا نعدّل الرصيد من هنا؛ استخدم «إضافة وارد» أو «صرف صنف» لتسجيل حركة صحيحة.</p></label> : <label><span className="field-label">الرصيد الافتتاحي</span><input type="number" min="0" className="field-input" value={openingQuantity} onChange={event => setOpeningQuantity(event.target.value)} required /></label>}<label><span className="field-label">الحد الأدنى للرصيد</span><input type="number" min="0" className="field-input" value={reorderLevel} onChange={event => setReorderLevel(event.target.value)} /><p className="mt-1 text-xs text-muted-foreground">يظهر تنبيه تلقائي عند وصول الرصيد إلى هذا الحد أو انخفاضه عنه.</p></label><label><span className="field-label">سعر شراء القطعة</span><input type="number" min="0" step="0.01" inputMode="decimal" className="field-input" value={defaultUnitCost} onChange={event => setDefaultUnitCost(event.target.value)} placeholder="0" /><p className="mt-1 text-xs text-muted-foreground">يُستخدم تلقائيًا عند تسجيل الوارد لخصم التكلفة من الخزينة.</p></label><label className="sm:col-span-2"><span className="field-label">ملاحظات</span><textarea className="field-textarea" value={itemNotes} onChange={event => setItemNotes(event.target.value)} placeholder="المقاس أو المورد أو أي ملاحظة مفيدة" /></label></div><div className="flex justify-end gap-3"><Button type="button" variant="outline" onClick={() => setItemDialog(false)} className="rounded-xl">إلغاء</Button><Button type="submit" disabled={createItem.isPending || updateItem.isPending} className="rounded-xl bg-teal-700 hover:bg-teal-800">{createItem.isPending || updateItem.isPending ? "جارٍ الحفظ…" : editingItemId ? "حفظ التعديل" : "إضافة الصنف"}</Button></div></form></DialogContent></Dialog>
...
function InventoryTableRow({ item, onMovement, onIncoming, onEdit, onDelete, selected }: { item: { id: number; name: string; category?: string | null; customEmoji?: string | null; imageUrl?: string | null; notes: string | null; openingQuantity: number; currentBalance: number; reorderLevel?: number | null; defaultUnitCost?: number | null; createdAt?: Date | string | null }; onMovement: () => void; onIncoming: () => void; onEdit: () => void; onDelete: () => void; selected?: boolean }) {
...
function MovementTableRow({ movement, onDelete }: { movement: { id: number; movementDate: Date; inventoryItemName: string; movementType: "incoming" | "outgoing"; quantity: number; unitCost?: number | null; technicianName: string | null; notes: string | null }; onDelete: () => void }) { const totalCost = (movement.unitCost ?? 0) * movement.quantity; return <tr><td className="px-5 py-4 text-sm">{formatDate(movement.movementDate)}</td><td className="px-5 py-4 font-bold text-teal-950">{movement.inventoryItemName}</td><td className="px-5 py-4"><MovementType movementType={movement.movementType} /></td><td className="px-5 py-4 font-extrabold">{movement.quantity}</td><td className="px-5 py-4 text-sm">{movement.movementType === "incoming" ? <div className="min-w-48 flex items-center justify-between gap-3 rounded-lg border border-violet-200 bg-violet-50 px-3 py-2"><div><p className="text-[10px] font-bold text-violet-700">سعر الوحدة</p><p className="mt-0.5 font-black text-violet-950">{formatMoney(movement.unitCost ?? 0)}</p></div><div className="h-7 w-px bg-violet-200" /><div className="text-left"><p className="text-[10px] font-bold text-violet-700">الإجمالي · {movement.quantity} قطعة</p><p className="mt-0.5 font-black text-violet-950">{formatMoney(totalCost)}</p></div></div> : <span className="text-muted-foreground">—</span>}</td><td className="px-5 py-4 text-sm">{movement.technicianName || "—"}</td><td className="max-w-64 truncate px-5 py-4 text-sm text-muted-foreground">{movement.notes || "—"}</td><td className="px-5 py-4"><Button size="sm" variant="outline" onClick={onDelete} className="rounded-lg border-rose-200 text-rose-700 hover:bg-rose-50">حذف</Button></td></tr>; }
