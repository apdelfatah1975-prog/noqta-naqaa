...
export default function Inventory() {
  ...
  const inventoryStats = useMemo(() => {
    ...
    return {
      ...
      purchaseCost: movements.filter(movement => movement.movementType === "incoming").reduce((sum, movement) => sum + movement.quantity * (movement.unitCost ?? 0), 0),
    };
  }, [data]);
  useEffect(() => {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <InventorySummaryCard label="إجمالي الأصناف" value={inventoryStats.totalItems.toLocaleString("ar-SA")} hint="الأصناف المسجلة" tone="teal" />
        <InventorySummaryCard label="رصيد منخفض" value={inventoryStats.lowStock.toLocaleString("ar-SA")} hint="أصناف تحتاج متابعة" tone="amber" />
        <InventorySummaryCard label="حركة الوارد / المنصرف" value={`${inventoryStats.incoming.toLocaleString("ar-SA")} / ${inventoryStats.outgoing.toLocaleString("ar-SA")}`} hint="بالقطعة خلال السجل" tone="sky" />
        <InventorySummaryCard label="تكلفة المشتريات" value={formatMoney(inventoryStats.purchaseCost)} hint="المبلغ المرتبط بالخزينة" tone="violet" />
      </div>

      <section className="soft-card overflow-hidden">
        <div className="hidden overflow-x-auto md:block">
          <table className="w-full min-w-[760px] text-right">
            <thead className="bg-teal-50/70 text-xs text-teal-950/65"><tr><th className="px-5 py-4 font-bold">رقم الصنف</th><th className="px-5 py-4 font-bold">نوع الصنف</th><th className="px-5 py-4 font-bold">الرصيد الافتتاحي</th><th className="px-5 py-4 font-bold">الرصيد الحالي</th><th className="px-5 py-4 font-bold">آخر سعر شراء للوحدة</th><th className="px-5 py-4 font-bold">الحالة</th><th className="px-5 py-4 font-bold">إجراء</th></tr></thead>
            <tbody className="divide-y divide-teal-950/6">
              {data?.items.length ? data.items.map(item => <InventoryTableRow key={item.id} item={item} unitCost={latestPurchaseUnitCost(item.id, data.movements)} onMovement={() => setMovementItem({ id: item.id, name: item.name })} onDelete={() => setPinAction({ kind: "item", id: item.id })} />) : <EmptyInventoryRow isLoading={isLoading} />}
            </tbody>
          </table>
        </div>
        <div className="divide-y divide-teal-950/6 md:hidden">
          {data?.items.length ? data.items.map(item => <div key={item.id} className="space-y-4 p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold tracking-wide text-teal-700">رقم الصنف: {inventoryItemCode(item.id)}</p><p className="mt-1 text-lg font-extrabold leading-7 text-teal-950">{item.name}</p><div className="mt-2 flex flex-wrap gap-2 text-[11px] font-bold"><span className="rounded-full bg-slate-100 px-2 py-1 text-slate-600">{item.category}</span><span className="rounded-full bg-teal-50 px-2 py-1 text-teal-700">الوحدة: {item.unit}</span><span className="rounded-full bg-amber-50 px-2 py-1 text-amber-700">تنبيه عند: {item.reorderLevel}</span></div>{item.notes ? <p className="mt-1 text-xs leading-5 text-muted-foreground">{item.notes}</p> : null}</div><StockBadge balance={item.currentBalance} /></div><div className="grid grid-cols-2 gap-3 rounded-xl bg-teal-50/60 p-3 text-sm"><div><p className="text-xs text-muted-foreground">الرصيد الافتتاحي</p><p className="mt-1 font-extrabold">{item.openingQuantity}</p></div><div><p className="text-xs text-muted-foreground">الرصيد الحالي</p><p className="mt-1 text-lg font-extrabold text-teal-800">{item.currentBalance}</p></div><div className="col-span-2 border-t border-teal-900/10 pt-2"><p className="text-xs text-muted-foreground">آخر سعر شراء للوحدة</p><p className="mt-1 font-extrabold text-violet-800">{formatMoney(latestPurchaseUnitCost(item.id, data.movements))}</p></div></div><div className="grid gap-2 sm:grid-cols-2"><Button size="sm" variant="outline" onClick={() => setMovementItem({ id: item.id, name: item.name })} className="w-full rounded-xl border-teal-700/20 text-teal-800 hover:bg-teal-50"><PackagePlus className="ml-1 h-4 w-4" />تسجيل حركة</Button><Button size="sm" variant="outline" onClick={() => setPinAction({ kind: "item", id: item.id })} className="w-full rounded-xl border-rose-200 text-rose-700 hover:bg-rose-50">حذف الصنف</Button></div></div>) : <EmptyInventoryCard isLoading={isLoading} />}
        </div>
      </section>
