    ...
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <section aria-labelledby="inventory-items-cards" className="rounded-2xl border border-teal-100 bg-teal-50/60 p-2.5 shadow-sm">
        ...
        {data.items.length ? <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8">
          {data.items.map(item => <button key={item.id} type="button" onClick={() => focusInventoryItem(item.id)} onKeyDown={event => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); focusInventoryItem(item.id); } }} className="inventory-item-card flex min-w-0 items-center gap-2 rounded-xl border border-white bg-white px-2 py-2 text-right shadow-sm transition duration-200 ease-out hover:-translate-y-0.5 hover:shadow-md active:scale-[0.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-500" aria-label={`الصنف ${item.name}، رقم المخزون ${item.id}، الرصيد ${item.currentBalance}`}>
      ...
      <section className="soft-card overflow-hidden">
        <div className="hidden overflow-x-auto md:block">
          <table className="w-full min-w-[760px] text-right">
            ...
            <tbody className="divide-y divide-teal-950/6">
              {data?.items.length ? data.items.map(item => <InventoryTableRow key={item.id} item={item} selected={item.id === selectedItemId} onMovement={() => openMovement({ id: item.id, name: item.name, defaultUnitCost: item.defaultUnitCost }, "outgoing")} onDelete={() => setPinAction({ kind: "item", id: item.id })} />) : <EmptyInventoryRow isLoading={isLoading} />}
            </tbody>
          </table>
        </div>
        ...
        <div className="divide-y divide-teal-950/6 md:hidden">
          {data?.items.length ? data.items.map(item => (
            <div id={`inventory-item-${item.id}`} key={item.id} className={`inventory-item-row space-y-4 p-5 transition-[background-color,box-shadow] duration-500 ease-out ${item.id === selectedItemId ? "inventory-item-row-selected bg-teal-50 ring-2 ring-inset ring-teal-300" : "hover:bg-teal-50/40"}`}>
...
function InventoryTableRow({ item, onMovement, onDelete, selected }: { item: { id: number; name: string; category?: string | null; customEmoji?: string | null; imageUrl?: string | null; notes: string | null; openingQuantity: number; currentBalance: number; reorderLevel?: number | null; defaultUnitCost?: number | null; createdAt?: Date | string | null }; onMovement: () => void; onDelete: () => void; selected?: boolean }) {
  return <tr id={`inventory-item-${item.id}`} className={`inventory-item-row transition-[background-color,box-shadow] duration-500 ease-out ${selected ? "inventory-item-row-selected bg-teal-50 ring-2 ring-inset ring-teal-300" : "hover:bg-teal-50/45"}`}>
