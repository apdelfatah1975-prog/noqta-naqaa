      ...
      <section className="soft-card overflow-hidden">
        ...
        <div className="divide-y divide-teal-950/6 md:hidden">
          {data?.items.length ? data.items.map(item => (
            <div id={`inventory-item-${item.id}`} key={item.id} className={`inventory-item-row space-y-4 p-5 transition-[background-color,box-shadow] duration-500 ease-out ${item.id === selectedItemId ? "inventory-item-row-selected bg-amber-100 ring-2 ring-inset ring-amber-400 shadow-[inset_0_0_0_1px_rgba(245,158,11,.35)]" : "hover:bg-teal-50/40"}`}>
...
function InventoryTableRow({ item, onMovement, onIncoming, onDelete, selected }: { item: { id: number; name: string; category?: string | null; customEmoji?: string | null; imageUrl?: string | null; notes: string | null; openingQuantity: number; currentBalance: number; reorderLevel?: number | null; defaultUnitCost?: number | null; createdAt?: Date | string | null }; onMovement: () => void; onIncoming: () => void; onDelete: () => void; selected?: boolean }) {
  return <tr id={`inventory-item-${item.id}`} className={`inventory-item-row transition-[background-color,box-shadow] duration-500 ease-out ${selected ? "inventory-item-row-selected bg-amber-100 ring-2 ring-inset ring-amber-400 shadow-[inset_0_0_0_1px_rgba(245,158,11,.35)]" : "hover:bg-teal-50/45"}`}>
