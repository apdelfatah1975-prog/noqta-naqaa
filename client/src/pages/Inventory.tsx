...
export default function Inventory() {
  ...
  const [movementCurrency, setMovementCurrency] = useState<"EGP" | "SAR">("EGP");
  ...
  const createMovement = trpc.filters.inventory.createMovement.useMutation({
    onSuccess: () => {
      ...
      setMovementItem(null); setQuantity(""); setUnitCost(""); setMovementCurrency("EGP"); setTechnicianName(""); setMovementNotes("");
    },
  ...
  function submitMovement(event: FormEvent) {
    ...
    createMovement.mutate({ inventoryItemId: movementItem.id, movementType, quantity: Number(quantity), unitCost: movementType === "incoming" ? Number(unitCost || 0) : 0, currency: movementCurrency, movementDate: new Date(movementDate), technicianName: technicianName || null, notes: movementNotes || null });
  }
