...
function InventoryTableRow({ item, onMovement, onDelete, selected }: { item: { id: number; name: string; category?: string | null; notes: string | null; openingQuantity: number; currentBalance: number }; onMovement: () => void; onDelete: () => void; selected?: boolean }) { return <tr id={`inventory-item-${item.id}`} className={selected ? "bg-teal-50 ring-2 ring-inset ring-teal-300" : "hover:bg-teal-50/45"}><td className="px-5 py-4 text-sm font-bold text-teal-700">{inventoryItemCode(item.id)}</td><td className="px-5 py-4"><p className="text-base font-extrabold text-teal-950">{item.name}</p><p className="mt-1 text-xs font-bold text-teal-700">{item.category || "عام"}</p>{item.notes ? <p className="mt-1 max-w-64 truncate text-xs text-muted-foreground">{item.notes}</p> : null}</td><td className="px-5 py-4">{item.openingQuantity}</td><td className="px-5 py-4 text-lg font-extrabold text-teal-800">{item.currentBalance}</td><td className="px-5 py-4"><StockBadge balance={item.currentBalance} /></td><td className="px-5 py-4"><Button size="sm" variant="outline" onClick={onMovement} className="rounded-lg border-teal-700/20 text-teal-800 hover:bg-teal-50"><PackagePlus className="ml-1 h-4 w-4" />صرف صنف</Button><Button size="sm" variant="outline" onClick={onDelete} className="mr-2 rounded-lg border-rose-200 text-rose-700 hover:bg-rose-50">حذف</Button></td></tr>; }
