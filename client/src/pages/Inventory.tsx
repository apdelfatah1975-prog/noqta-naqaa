  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <section className="soft-card overflow-hidden">
        ...
        <div className="divide-y divide-teal-950/6 md:hidden">
          {data?.movements.length ? data.movements.map(movement => <div key={movement.id} className="p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-xs text-muted-foreground">الصنف</p><p className="mt-1 font-extrabold text-teal-950">{movement.inventoryItemName}</p></div><span className="font-extrabold">{movement.quantity} قطعة</span></div><div className="mt-3"><MovementType movementType={movement.movementType} /></div><div className="mt-3 grid grid-cols-2 gap-2 text-xs"><div className="rounded-xl bg-slate-50 p-2.5"><p className="text-[11px] font-bold text-slate-500">التاريخ</p><p className="mt-1 font-bold text-teal-950">{formatDate(movement.movementDate)}</p></div>{movement.movementType === "incoming" ? <><div className="rounded-xl bg-violet-50 p-2.5"><p className="text-[11px] font-bold text-violet-700">سعر الوحدة</p><p className="mt-1 font-black text-violet-950">{formatMoney(movement.unitCost ?? 0)}</p></div><div className="col-span-2 flex items-center justify-between rounded-xl border border-violet-200 bg-violet-100/70 px-3 py-2.5"><span className="font-bold text-violet-800">إجمالي التكلفة</span><strong className="text-base font-black text-violet-950">{formatMoney((movement.unitCost ?? 0) * movement.quantity)}</strong></div></> : null}<div className="rounded-xl bg-slate-50 p-2.5"><p className="text-[11px] font-bold text-slate-500">الفني / المستلم</p><p className="mt-1 font-bold text-teal-950">{movement.technicianName || "—"}</p></div>{movement.notes ? <div className="col-span-2 rounded-xl bg-slate-50 p-2.5"><p className="text-[11px] font-bold text-slate-500">ملاحظات</p><p className="mt-1 font-bold text-teal-950">{movement.notes}</p></div> : null}</div><Button size="sm" variant="outline" onClick={() => setPinAction({ kind: "movement", id: movement.id })} className="mt-4 w-full rounded-xl border-rose-200 text-rose-700 hover:bg-rose-50">حذف الحركة</Button></div>) : <div className="p-10 text-center text-sm text-muted-foreground">لا توجد حركات في المخزن بعد.</div>}
        </div>
      </section>
...
function MovementTableRow({ movement, onDelete }: { movement: { id: number; movementDate: Date; inventoryItemName: string; movementType: "incoming" | "outgoing"; quantity: number; unitCost?: number | null; technicianName: string | null; notes: string | null }; onDelete: () => void }) { const totalCost = (movement.unitCost ?? 0) * movement.quantity; return <tr><td className="px-5 py-4 text-sm">{formatDate(movement.movementDate)}</td><td className="px-5 py-4 font-bold text-teal-950">{movement.inventoryItemName}</td><td className="px-5 py-4"><MovementType movementType={movement.movementType} /></td><td className="px-5 py-4 font-extrabold">{movement.quantity}</td><td className="px-5 py-4 text-sm">{movement.movementType === "incoming" ? <div className="min-w-44 space-y-2"><div className="rounded-lg bg-violet-50 px-3 py-2"><p className="text-[11px] font-bold text-violet-700">سعر الوحدة</p><p className="mt-0.5 font-black text-violet-950">{formatMoney(movement.unitCost ?? 0)}</p></div><div className="rounded-lg border border-violet-200 bg-violet-100/70 px-3 py-2"><p className="text-[11px] font-bold text-violet-800">الإجمالي · {movement.quantity} قطعة</p><p className="mt-0.5 font-black text-violet-950">{formatMoney(totalCost)}</p></div></div> : <span className="text-muted-foreground">—</span>}</td><td className="px-5 py-4 text-sm">{movement.technicianName || "—"}</td><td className="max-w-64 truncate px-5 py-4 text-sm text-muted-foreground">{movement.notes || "—"}</td><td className="px-5 py-4"><Button size="sm" variant="outline" onClick={onDelete} className="rounded-lg border-rose-200 text-rose-700 hover:bg-rose-50">حذف</Button></td></tr>; }
