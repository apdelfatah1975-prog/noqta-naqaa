...
import { emptyTrash, getTrashItems, permanentlyDeleteFromTrash, restoreFromTrash, moveToTrash, type TrashItem } from "@/lib/trashBin";
...
export default function Settings() {
  ...
  return <div className="mx-auto max-w-5xl space-y-6" dir="rtl">
    ...
    <section ref={trashSectionRef} id="trash-bin" className="soft-card scroll-mt-4 overflow-hidden"><div className="flex flex-wrap items-start justify-between gap-3 border-b border-teal-950/6 p-5"><div><h2 className="font-extrabold">سلة المحذوفات</h2><p className="mt-1 text-xs leading-6 text-muted-foreground">العناصر المنقولة إلى هنا تبقى محفوظة على هذا الجهاز دون إنترنت حتى تستعيدها أو تحذفها نهائيًا.</p></div><div className="flex items-center gap-2"><span className="rounded-full bg-rose-50 px-3 py-2 text-xs font-bold text-rose-800">{trashItems.length} عنصر</span>{trashItems.length > 0 && <Button type="button" size="sm" variant="outline" className="rounded-lg border-rose-300 text-rose-800 hover:bg-rose-50" onClick={securelyEmptyTrash} disabled={verifyPin.isPending}><Trash2 className="ml-1 h-4 w-4" />{verifyPin.isPending ? "جارٍ التحقق..." : "إفراغ السلة بالكامل"}</Button>}</div></div><div className="space-y-2 p-5">{trashItems.length ? trashItems.map(item => <div key={item.id} className="flex flex-col gap-3 rounded-xl border border-rose-100 bg-rose-50/50 p-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-bold text-rose-950">{item.entityLabel}</p><p className="mt-1 text-xs leading-6 text-rose-800">حُذف بواسطة <span className="font-bold">{item.deletedBy || "مستخدم سابق"}</span> في {new Date(item.deletedAt).toLocaleString("ar-SA", { dateStyle: "medium", timeStyle: "short" })}</p></div><div className="flex flex-wrap gap-2"><Button type="button" size="sm" className="rounded-lg bg-teal-700 hover:bg-teal-800" onClick={() => restoreTrashItem(item)}><Undo2 className="ml-1 h-4 w-4" />استعادة</Button><Button type="button" size="sm" variant="outline" className="rounded-lg border-rose-300 text-rose-800" onClick={() => permanentlyDeleteTrashItem(item)}><Trash2 className="ml-1 h-4 w-4" />حذف نهائي</Button></div></div>) : <p className="rounded-xl bg-slate-50 p-4 text-center text-sm text-muted-foreground">سلة المحذوفات فارغة حاليًا.</p>}</div></section>
