...
export default function Settings() {
  ...
  function createPerformanceCustomers() {
    if (!window.confirm("سيتم إنشاء 1000 عميل تجريبي بأسماء تبدأ بـ عميل تجريبي للأداء. لن تُنشأ لهم زيارات أو معاملات مالية. هل تريد المتابعة؟")) return;
  ...
  return <div className="mx-auto max-w-5xl space-y-6" dir="rtl">
    ...
    <section className="soft-card overflow-hidden border border-amber-200"><div className="flex items-start gap-3 border-b border-amber-100 bg-amber-50/60 p-5"><div className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-amber-100 text-amber-700"><span className="text-lg font-black">1000</span></div><div><h2 className="font-extrabold text-amber-950">اختبار أداء التطبيق</h2><p className="mt-1 text-xs leading-6 text-amber-900">ينشئ هذا الاختبار 1000 عميل تجريبي موسومًا بوضوح، دون زيارات أو معاملات مالية، حتى تختبر سرعة فتح صفحة العملاء والبحث والتنقل. احذفهم من هنا بعد انتهاء الاختبار.</p></div></div><div className="flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between"><p className="text-sm font-semibold text-slate-700">العملاء التجريبيون يبدأ اسمهم بـ «عميل تجريبي للأداء» وكودهم بـ PERF-.</p><div className="flex flex-wrap gap-2"><Button type="button" className="rounded-xl bg-amber-700 hover:bg-amber-800" onClick={createPerformanceCustomers} disabled={seedPerformanceCustomers.isPending}>{seedPerformanceCustomers.isPending ? "جارٍ الإنشاء…" : "إنشاء 1000 عميل تجريبي"}</Button><Button type="button" variant="outline" className="rounded-xl border-rose-300 text-rose-800 hover:bg-rose-50" onClick={removePerformanceCustomers} disabled={deletePerformanceCustomers.isPending}>{deletePerformanceCustomers.isPending ? "جارٍ الحذف…" : "حذف العملاء التجريبيين"}</Button></div></div></section>

    <section className="soft-card overflow-hidden border border-rose-200"><div className="flex items-start gap-3 border-b border-rose-100 bg-rose-50/50 p-5"><div className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-rose-100 text-rose-700"><Trash2 className="h-5 w-5" /></div><div><h2 className="font-extrabold text-rose-950">منطقة الحذف المتقدم</h2><p className="mt-1 text-xs leading-6 text-rose-800">حذف جميع العملاء والزيارات والتذكيرات المرتبطة نهائيًا. لن تتأثر الخزنة أو المخزن، ولا يمكن التراجع عن هذه العملية.</p></div></div><div className="flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between"><p className="text-sm font-semibold text-slate-700">استخدم هذا الإجراء فقط عند بدء قاعدة بيانات جديدة أو بعد أخذ نسخة احتياطية.</p><Button type="button" variant="outline" className="rounded-xl border-rose-300 font-bold text-rose-800 hover:bg-rose-50" onClick={() => setBulkDeleteOpen(true)}><Trash2 className="ml-2 h-4 w-4" />حذف جميع العملاء</Button></div></section>
