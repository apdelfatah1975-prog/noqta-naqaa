...
export default function Settings() {
  ...
  const pendingReviews = trpc.filters.reviews.pending.useQuery(undefined, { retry: false, staleTime: 30_000 });
  const approveReview = trpc.filters.reviews.approve.useMutation({ onSuccess: () => { void pendingReviews.refetch(); toast.success("تم اعتماد التقييم ونشره للزوار"); }, onError: error => toast.error(error.message || "تعذر اعتماد التقييم") });
  const rejectReview = trpc.filters.reviews.reject.useMutation({ onSuccess: () => { void pendingReviews.refetch(); toast.success("تم رفض التقييم وإخفاؤه"); }, onError: error => toast.error(error.message || "تعذر رفض التقييم") });
  ...
  return <div className="mx-auto max-w-5xl space-y-6" dir="rtl">
    ...
    <section className="soft-card overflow-hidden"><div className="border-b border-teal-950/6 p-5"><h2 className="font-extrabold">مراجعة آراء العملاء</h2><p className="mt-1 text-xs leading-6 text-muted-foreground">لا تظهر أي مشاركة في الموقع العام قبل اعتمادها يدويًا من الإدارة.</p></div><div className="space-y-3 p-5">{pendingReviews.isLoading ? <p className="rounded-xl bg-slate-50 p-4 text-sm text-muted-foreground">جارٍ تحميل التقييمات المعلقة...</p> : pendingReviews.data?.length ? pendingReviews.data.map(review => <article key={review.id} className="rounded-2xl border border-amber-200 bg-amber-50/50 p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-extrabold text-teal-950">{review.customerName}</h3><span className="rounded-full bg-amber-100 px-2.5 py-1 text-xs font-black text-amber-800">{review.stars} / 5 نجوم</span></div><p className="mt-2 text-sm leading-7 text-slate-700">{review.comment}</p><p className="mt-2 text-xs text-muted-foreground">أُرسل في {new Date(review.createdAt).toLocaleString("ar-SA")}</p></div><div className="flex shrink-0 gap-2"><Button type="button" size="sm" disabled={approveReview.isPending || rejectReview.isPending} onClick={() => approveReview.mutate({ id: review.id })} className="rounded-xl bg-teal-700 hover:bg-teal-800">اعتماد ونشر</Button><Button type="button" size="sm" variant="outline" disabled={approveReview.isPending || rejectReview.isPending} onClick={() => rejectReview.mutate({ id: review.id })} className="rounded-xl border-rose-200 text-rose-700 hover:bg-rose-50">رفض</Button></div></div></article>) : <p className="rounded-xl bg-emerald-50 p-4 text-sm font-bold text-emerald-800">لا توجد تقييمات معلقة للمراجعة.</p>}</div></section>
