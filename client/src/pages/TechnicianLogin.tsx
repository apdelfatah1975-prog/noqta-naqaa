        ...
        <div className="grid w-full overflow-hidden rounded-[2rem] border border-teal-100 bg-white shadow-[0_24px_70px_rgba(15,118,110,0.14)] lg:grid-cols-[0.9fr_1.1fr]">
          ...
          <section className="p-6 sm:p-10 lg:p-14">
            ...
            <form className="space-y-5" onSubmit={event => { event.preventDefault(); login.mutate({ email: email.trim(), password }); }}>
              ...
              <div className="space-y-2"><Label htmlFor="technician-login-password" className="font-black">كلمة السر</Label><div className="relative"><KeyRound className="pointer-events-none absolute right-3 top-3 h-5 w-5 text-slate-400" /><Input id="technician-login-password" className="h-12 rounded-xl border-slate-200 bg-slate-50 pr-10 font-semibold focus-visible:ring-teal-500" type="password" dir="ltr" value={password} onChange={event => setPassword(event.target.value)} placeholder="أدخل كلمة السر" autoComplete="current-password" required minLength={8} /></div></div>
            ...
            <p className="mt-6 text-center text-xs font-bold leading-6 text-slate-400">هذه الواجهة مخصصة لأوامر العمل فقط، ولا تعرض الخزينة أو التقارير أو إعدادات الإدارة.</p>
          </section>
        </div>
      </div>
    </main>
