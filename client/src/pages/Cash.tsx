...
const formatMoney = (amount: number) => new Intl.NumberFormat("ar-EG", { style: "currency", currency: "EGP", maximumFractionDigits: 0 }).format(amount / 100);
...
export default function Cash() {
  ...
  const summaryCards = [
    { label: "إجمالي الإيرادات", amount: data?.incomeTotal ?? 0, icon: ArrowDownRight, tone: "bg-teal-50 text-teal-800" },
    { label: "إجمالي المصروفات", amount: data?.expenseTotal ?? 0, icon: ArrowUpLeft, tone: "bg-amber-50 text-amber-800" },
    { label: "رصيد الخزينة", amount: data?.balance ?? 0, icon: WalletCards, tone: "bg-slate-950 text-white" },
  ];
  ...
  return <div className="mx-auto max-w-7xl space-y-6">
    ...
    <section className="grid gap-4 sm:grid-cols-3">
      {summaryCards.map(card => <article key={card.label} className={`rounded-2xl p-5 shadow-sm ${card.tone}`}><div className="flex items-center justify-between"><p className="text-sm font-bold opacity-80">{card.label}</p><card.icon className="h-5 w-5 opacity-80" /></div><p className="mt-4 text-2xl font-extrabold tracking-tight">{formatMoney(card.amount)}</p></article>)}
    </section>
...
function CashTableRow({ transaction }: { transaction: CashTransaction }) { return <tr><td className="px-5 py-4 text-sm">{formatDate(transaction.transactionDate)}</td><td className="px-5 py-4"><TransactionBadge type={transaction.transactionType} /></td><td className="px-5 py-4 font-bold">{transaction.category}</td><td className={`px-5 py-4 font-extrabold ${transaction.transactionType === "income" ? "text-teal-700" : "text-amber-700"}`}>{transaction.transactionType === "income" ? "+" : "−"}{formatMoney(transaction.amount)}</td><td className="px-5 py-4 text-sm">{transaction.recipientName || "—"}</td><td className="max-w-64 truncate px-5 py-4 text-sm text-muted-foreground">{transaction.notes || "—"}</td></tr>; }
function CashCard({ transaction }: { transaction: CashTransaction }) { return <div className="p-5"><div className="flex items-start justify-between gap-3"><div><p className="font-extrabold">{transaction.category}</p><p className="mt-1 text-xs text-muted-foreground">{formatDate(transaction.transactionDate)}</p></div><TransactionBadge type={transaction.transactionType} /></div><p className={`mt-4 text-xl font-extrabold ${transaction.transactionType === "income" ? "text-teal-700" : "text-amber-700"}`}>{transaction.transactionType === "income" ? "+" : "−"}{formatMoney(transaction.amount)}</p><div className="mt-3 grid grid-cols-2 gap-y-2 text-xs text-muted-foreground"><p>الفني / الجهة</p><p className="text-left text-teal-950">{transaction.recipientName || "—"}</p>{transaction.notes ? <><p>ملاحظات</p><p className="text-left text-teal-950">{transaction.notes}</p></> : null}</div></div>; }
function EmptyCashRow({ isLoading }: { isLoading: boolean }) { return <tr><td colSpan={6} className="p-14 text-center"><CircleDollarSign className="mx-auto h-8 w-8 text-teal-200" /><p className="mt-3 text-sm text-muted-foreground">{isLoading ? "جارٍ تحميل الخزينة…" : "لا توجد عمليات مالية حتى الآن."}</p></td></tr>; }
