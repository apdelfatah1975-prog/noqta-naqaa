...
export default function Cash() {
  ...
  const summaries = data?.summaries ?? { SAR: { incomeTotal: data?.incomeTotal ?? 0, expenseTotal: data?.expenseTotal ?? 0, balance: data?.balance ?? 0 }, EGP: { incomeTotal: 0, expenseTotal: 0, balance: 0 } };
  const emptyAnalytics = { installationIncome: 0, serviceIncome: 0, expenseByCategory: [], technicianExpenses: [] };
  const breakdown = data?.breakdown ?? { EGP: { income: [], expense: [], analytics: emptyAnalytics }, SAR: { income: [], expense: [], analytics: emptyAnalytics } };
  const purchases = data?.purchases ?? { EGP: { total: 0, items: [] }, SAR: { total: 0, items: [] } };
  ...
  return <div className="mx-auto max-w-7xl space-y-6">
    ...
    <section className="soft-card p-5">
      ...
      <div className="grid gap-5 lg:grid-cols-2">{(["SAR", "EGP"] as Currency[]).map(cardCurrency => <PurchasePanel key={cardCurrency} currency={cardCurrency} purchase={purchases[cardCurrency]} />)}</div>
    </section>
    ...
    <section className="soft-card p-5">
      ...
      <div className="grid gap-5 lg:grid-cols-2">{(["SAR", "EGP"] as Currency[]).map(cardCurrency => <AnalyticsPanel key={cardCurrency} currency={cardCurrency} analytics={breakdown[cardCurrency].analytics} />)}</div>
    </section>
