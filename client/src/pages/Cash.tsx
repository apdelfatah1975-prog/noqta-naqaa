...
export default function Cash() {
  ...
  return <div className="mx-auto max-w-7xl space-y-6">
    ...
    <section className="soft-card p-5">
      <div className="mb-5 flex items-center justify-between"><div><h2 className="font-extrabold">المشتريات حسب الصنف</h2><p className="mt-1 text-xs text-muted-foreground">إجمالي البضائع المشتراة وتكلفة كل صنف ضمن الفلاتر الحالية.</p></div><ReceiptText className="h-5 w-5 text-indigo-700" /></div>
      <div className="grid gap-5 lg:grid-cols-2">{(["SAR"] as Currency[]).map(cardCurrency => <PurchasePanel key={cardCurrency} currency={cardCurrency} purchase={purchases[cardCurrency]} />)}</div>
    </section>

    <section className="soft-card p-5">
      <div className="mb-5 flex items-center justify-between"><div><h2 className="font-extrabold">تحليل الإيرادات والمصروفات</h2><p className="mt-1 text-xs text-muted-foreground">النتائج حسب الفلاتر الحالية. ابحث باسم الفني لمعرفة إجمالي ما صُرف له.</p></div><CircleDollarSign className="h-5 w-5 text-teal-700" /></div>
      <div className="grid gap-5 lg:grid-cols-2">{(["SAR"] as Currency[]).map(cardCurrency => <AnalyticsPanel key={cardCurrency} currency={cardCurrency} analytics={breakdown[cardCurrency].analytics} />)}</div>
    </section>
