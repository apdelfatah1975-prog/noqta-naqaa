...
import { ArrowDownRight, ArrowUpLeft, CircleDollarSign, Download, Plus, ReceiptText, Search, SlidersHorizontal, WalletCards, X } from "lucide-react";
...
import { downloadRowsAsExcel, withArabicHeaders } from "@/lib/excelExport";
...
const formatMoney = (amount: number, _currency: Currency = "SAR") => new Intl.NumberFormat("ar-EG", { maximumFractionDigits: 0 }).format(amount / 100);
...
export default function Cash() {
  ...
  const [paymentMethod, setPaymentMethod] = useState<"نقدي" | "تحويل بنكي" | "بطاقة" | "آجل">("نقدي");
  ...
  const createTransaction = trpc.filters.cash.create.useMutation({
    onSuccess: () => {
      ...
      setOpen(false); setAmount(""); setCategory(""); setRecipientName(""); setPaymentMethod("نقدي"); setNotes("");
    },
  ...
  const cashReportRows = (data?.transactions ?? []).map((transaction: CashTransaction) => ({
    date: formatDate(transaction.transactionDate),
    type: transaction.transactionType === "income" ? "إيراد" : "مصروف",
    category: transaction.category,
    amount: formatMoney(transaction.amount),
    purchase: transaction.purchaseDetails ? `${transaction.purchaseDetails.itemName} — ${transaction.purchaseDetails.quantity} × ${formatMoney(transaction.purchaseDetails.unitCost)} = ${formatMoney(transaction.purchaseDetails.total)}` : "—",
    recipient: transaction.recipientName || "—",
    paymentMethod: transaction.paymentMethod || "نقدي",
    notes: transaction.notes || "—",
  }));
  const cashReportHeaders = { date: "التاريخ", type: "النوع", category: "التصنيف", amount: "المبلغ", purchase: "تفاصيل شراء المخزن", recipient: "الفني / الجهة", paymentMethod: "طريقة الدفع", notes: "ملاحظات" };
  function exportCashExcel() {
    if (!cashReportRows.length) { toast.info("لا توجد عمليات مطابقة للتصدير"); return; }
    downloadRowsAsExcel(`خزينة-نقطة-نقاء-${new Date().toISOString().slice(0, 10)}.xlsx`, "الخزينة", withArabicHeaders(cashReportRows, cashReportHeaders));
    toast.success("تم تجهيز تقرير الخزينة Excel");
  }
  function exportCashPdf() {
    if (!cashReportRows.length) { toast.info("لا توجد عمليات مطابقة للتصدير"); return; }
    const opened = printArabicPdf("تقرير الخزينة", cashReportRows, Object.entries(cashReportHeaders).map(([key, label]) => ({ key, label })));
    if (opened) toast.success("تم فتح تقرير الخزينة PDF للطباعة أو الحفظ"); else toast.error("تعذر فتح نافذة PDF. اسمح بالنوافذ المنبثقة ثم حاول مرة أخرى");
  }
...
function CashTableRow({ transaction, onDelete }: { transaction: CashTransaction; onDelete: () => void }) { const transactionCurrency: Currency = "SAR"; return <tr><td className="px-5 py-4 text-sm">{formatDate(transaction.transactionDate)}</td><td className="px-5 py-4"><TransactionBadge type={transaction.transactionType} /></td><td className="px-5 py-4 font-bold">{transaction.category}</td><td className={`px-5 py-4 font-extrabold ${transaction.transactionType === "income" ? "text-teal-700" : "text-amber-700"}`}>{transaction.transactionType === "income" ? "+" : "−"}{formatMoney(transaction.amount, transactionCurrency)}</td><td className="px-5 py-4 text-sm">{transaction.purchaseDetails ? <div className="min-w-44 rounded-xl bg-indigo-50/70 p-2 text-xs"><p className="font-extrabold text-indigo-950">{transaction.purchaseDetails.itemName}</p><p className="mt-1 text-indigo-950/70">{transaction.purchaseDetails.quantity} × {formatMoney(transaction.purchaseDetails.unitCost, transactionCurrency)}</p><p className="mt-1 font-extrabold text-indigo-700">الإجمالي: {formatMoney(transaction.purchaseDetails.total, transactionCurrency)}</p></div> : "—"}</td><td className="px-5 py-4 text-sm">{transaction.recipientName || "—"}</td><td className="px-5 py-4 text-sm"><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-bold text-slate-700">{transaction.paymentMethod || "نقدي"}</span></td><td className="max-w-64 truncate px-5 py-4 text-sm text-muted-foreground">{transaction.notes || "—"}</td><td className="px-5 py-4"><Button size="sm" variant="outline" onClick={onDelete} className="rounded-lg border-rose-200 text-rose-700 hover:bg-rose-50">حذف</Button></td></tr>; }
function CashCard({ transaction, onDelete }: { transaction: CashTransaction; onDelete: () => void }) { const transactionCurrency: Currency = "SAR"; return <div className="p-5"><div className="flex items-start justify-between gap-3"><div><p className="font-extrabold">{transaction.category}</p><p className="mt-1 text-xs text-muted-foreground">{formatDate(transaction.transactionDate)}</p></div><TransactionBadge type={transaction.transactionType} /></div><p className={`mt-4 text-xl font-extrabold ${transaction.transactionType === "income" ? "text-teal-700" : "text-amber-700"}`}>{transaction.transactionType === "income" ? "+" : "−"}{formatMoney(transaction.amount, transactionCurrency)}</p>{transaction.purchaseDetails ? <div className="mt-3 rounded-2xl bg-indigo-50/70 p-3"><p className="text-xs font-extrabold text-indigo-950">تفاصيل شراء المخزن</p><p className="mt-1 font-bold text-indigo-950">{transaction.purchaseDetails.itemName}</p><div className="mt-2 grid grid-cols-3 gap-2 text-[11px]"><div><p className="text-indigo-950/60">الكمية</p><p className="font-extrabold text-indigo-950">{transaction.purchaseDetails.quantity}</p></div><div><p className="text-indigo-950/60">سعر الوحدة</p><p className="font-extrabold text-indigo-950">{formatMoney(transaction.purchaseDetails.unitCost, transactionCurrency)}</p></div><div><p className="text-indigo-950/60">الإجمالي</p><p className="font-extrabold text-indigo-700">{formatMoney(transaction.purchaseDetails.total, transactionCurrency)}</p></div></div></div> : null}<div className="mt-3 grid grid-cols-2 gap-y-2 text-xs text-muted-foreground"><p>الفني / الجهة</p><p className="text-left text-teal-950">{transaction.recipientName || "—"}</p><p>طريقة الدفع</p><p className="text-left font-bold text-teal-950">{transaction.paymentMethod || "نقدي"}</p>{transaction.notes ? <><p>ملاحظات</p><p className="text-left text-teal-950">{transaction.notes}</p></> : null}</div><Button size="sm" variant="outline" onClick={onDelete} className="mt-4 w-full rounded-xl border-rose-200 text-rose-700 hover:bg-rose-50">حذف العملية</Button></div>; }
...
function TechnicianPaymentsPanel({ rows }: { rows: TechnicianPaymentRow[] }) {
  return <section className="soft-card p-5" aria-label="الراتب المدفوع لكل فني"><div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-extrabold">الراتب المدفوع لكل فني</h2><p className="mt-1 text-xs text-muted-foreground">يعرض راتب الفني المدفوع فعليًا فقط، ويتغير حسب الفترة والفلاتر المحددة في سجل العمليات.</p></div><Badge className="w-fit bg-teal-100 text-teal-800 hover:bg-teal-100">{rows.length} فني</Badge></div>{rows.length ? <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{rows.map(row => <article key={row.technician} className="rounded-2xl border border-teal-950/8 bg-teal-50/35 p-4"><div className="flex items-start justify-between gap-3"><h3 className="font-extrabold text-teal-950">{row.technician}</h3><Badge className={row.status === "paid" ? "bg-emerald-100 text-emerald-800 hover:bg-emerald-100" : "bg-amber-100 text-amber-800 hover:bg-amber-100"}>{row.status === "paid" ? "مدفوع" : "متبقي"}</Badge></div><div className="mt-4 grid grid-cols-2 gap-3 text-sm"><div><p className="text-xs text-muted-foreground">المستحق</p><p className="mt-1 font-extrabold">{formatMoney(row.requiredAmount)}</p></div><div><p className="text-xs text-muted-foreground">الراتب المدفوع فعليًا</p><p className="mt-1 font-extrabold text-teal-700">{formatMoney(row.salaryPaidAmount)}</p></div><div className="col-span-2 border-t border-teal-950/8 pt-3"><p className="text-xs text-muted-foreground">المتبقي</p><p className={`mt-1 font-extrabold ${row.remainingAmount > 0 ? "text-amber-700" : "text-emerald-700"}`}>{formatMoney(row.remainingAmount)}</p></div></div></article>)}</div> : <div className="mt-4 rounded-xl bg-slate-50 px-4 py-5 text-center text-sm text-muted-foreground">لا توجد دفعات فنيين ضمن الفلاتر الحالية.</div>}</section>;
}
...
function EmptyCashRow({ isLoading }: { isLoading: boolean }) { return <tr><td colSpan={9} className="p-14 text-center"><CircleDollarSign className="mx-auto h-8 w-8 text-teal-200" /><p className="mt-3 text-sm text-muted-foreground">{isLoading ? "جارٍ تحميل الخزينة…" : "لا توجد عمليات مالية حتى الآن."}</p></td></tr>; }
