...
import { BellRing, CheckCircle2, Clock3, Settings2, ShieldCheck, Volume2, VolumeX } from "lucide-react";
...
export function NotificationSettingsCard() {
  ...
  const setPin = trpc.filters.notifications.setPin.useMutation({
    onSuccess: () => { setNewPin(""); setCurrentPin(""); toast.success("تم حفظ الرقم السري لحماية التعديل والحذف"); },
    onError: error => toast.error(error.message || "تعذر حفظ الرقم السري."),
  });
  ...
  function savePin(event: React.FormEvent) { event.preventDefault(); if (newPin.trim().length < 4) { toast.error("اكتب رقمًا سريًا من 4 أحرف أو أرقام على الأقل."); return; } setPin.mutate({ newPin: newPin.trim(), currentPin: currentPin.trim() || undefined }); }
  ...
  return <section className="soft-card overflow-hidden">
    ...
    <div className="border-t border-teal-950/6 bg-slate-50/70 p-5"><div className="mb-3 flex items-center gap-2"><ShieldCheck className="h-5 w-5 text-teal-700" /><div><h3 className="font-extrabold">حماية التعديل والحذف</h3><p className="text-xs text-muted-foreground">أنشئ رقمًا سريًا لحماية بيانات العملاء والزيارات والمخزون والخزينة.</p></div></div><form onSubmit={savePin} className="grid gap-3 md:grid-cols-[1fr_1fr_auto]"><input type="password" className="field-input" value={currentPin} onChange={event => setCurrentPin(event.target.value)} placeholder="الرقم السري الحالي عند التغيير" autoComplete="current-password" /><input type="password" className="field-input" value={newPin} onChange={event => setNewPin(event.target.value)} placeholder="رقم سري جديد (4 أحرف على الأقل)" autoComplete="new-password" required minLength={4} /><Button type="submit" disabled={setPin.isPending} className="rounded-xl bg-teal-700 hover:bg-teal-800">{setPin.isPending ? "جارٍ الحفظ…" : "حفظ الرقم السري"}</Button></form><label className="mt-3 block max-w-sm"><span className="field-label">رقم سري لتعديل وقت التنبيه عند الحاجة</span><input type="password" className="field-input" value={settingsPin} onChange={event => setSettingsPin(event.target.value)} placeholder="يُطلب فقط إذا كان الرقم السري مضبوطًا" autoComplete="current-password" /></label></div>
  </section>;
}
