...
export function NotificationSettingsCard() {
  ...
  return <section className="soft-card overflow-hidden">
    ...
    <div className="border-t border-teal-950/6 bg-slate-50/70 p-5"><label className="block max-w-sm"><span className="field-label">رقم سري لتعديل وقت التنبيه عند الحاجة</span><input type="password" className="field-input" value={settingsPin} onChange={event => setSettingsPin(event.target.value)} placeholder="يُطلب فقط إذا كان الرقم السري مضبوطًا" autoComplete="current-password" /></label></div>
  </section>;
}
