...
export function CustomerContactActions({
  ...
  const locationRequestUrl = phone ? buildWhatsAppUrl(phone, `مرحبًا ${customer.name?.trim() || "عميلنا العزيز"}، نرجو إرسال موقعكم الجغرافي لتسهيل وصول الفني.`) : null;
  ...
  return <>
    <div className={`flex flex-wrap items-center gap-2.5 ${className}`} aria-label="إجراءات التواصل مع العميل">
      ...
      {labels && locationRequestUrl ? <a href={locationRequestUrl} className={`${baseClass} bg-cyan-50 text-cyan-800`} title="طلب موقع العميل عبر واتساب" aria-label="طلب موقع العميل عبر واتساب">{content(<MapPinned className={iconClass} aria-hidden="true" />, "طلب الموقع")}</a> : null}
    </div>
